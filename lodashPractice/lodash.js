...
let result = customers.map((elem) => elem.email);
console.log(result);

let result2 = _.map(customers, 'email');
console.log(result2);
...
let result3 = inviteList1.concat(inviteList2)
						 .filter((elem, i, arr) => (!arr.includes(elem, i + 1)) ? elem : null);
let result4 = _.union(inviteList1, inviteList2);
...
let mutual1 = friendsOfJim.concat(friendsOfBetty)
						  .filter((elem, i, arr) => (arr.includes(elem, i + 1)) ? elem : null);
let mutual2 = _.intersection(friendsOfJim, friendsOfBetty);
...
let groupedByPurchase = _.groupBy(purchases, 'company');

console.log(groupedByPurchase);